const we = {
	...
	info: {
		...
		soundChanges: [
			...
			"",
			"The first box is the _$t(search expression)_, the second is",
			"the _$t(replacement expression)_, the third is the",
			"_$t(context expression)_, and the last is the",
			"_$t(exception expression)_.",
			"",
			"The _$t(search expression)_ can include plain text or",
			...
			"$t(run).)",
			"",
			"The _$t(replacement expression)_ should be plain text. However, it",
			"can include non-negative $t(GR_other) **if and only if** the",
			"_$t(search expression)_ does, too. In that case, something",
			"special happens: when the evolver matches a character in a",
			"$t(charGroup), it will note what position that character is in",
			"the $t(charGroup)'s $t(run). It will then look at the _replacement_",
			...
			"the _$t(search expression)_ and _$t(replacement expression)_,",
			"errors may occur.",
			"",
			"The _$t(context expression)_ describes where in the word the",
			"_$t(search expression)_ must be before it can be changed into",
			"the _$t(replacement expression)_. The _$t(exception expression)_ is",
			...
			"underscore _ represents where the _$t(replacement expression)_ is",
