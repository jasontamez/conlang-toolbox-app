const we = {
	...
	info: {
		...
		transformations: [
			...
			"beginning and ending expressions, errors may occur.",
			"",
		...
		soundChanges: [
			...
			"",
			"The first box is the _$t(beginning expression)_, the second is",
			"the _$t(ending expression)_, the third is the",
			"_$t(context expression)_, and the last is the",
			"_$t(exception expression)_.",
			"",
			"The _$t(beginning expression)_ can include plain text or",
			...
			"$t(run).)",
			"",
			"The _$t(ending expression)_ should be plain text. However, it",
			"can include non-negative $t(GR_other) **if and only if** the",
			"_$t(beginning expression)_ does, too. In that case, something",
			"special happens: when the evolver matches a character in a",
			"$t(charGroup), it will note what position that character is in",
			"the $t(charGroup)'s $t(run). It will then look at the _ending_",
			"$t(charGroup) and pick out the character in the same position.",
			"For example: If %S is being replaced with %Z, and those",
			"$t(charGroup_other) have $t(runs) \"ptk\" and \"bdg\", \"p\"",
			"will be replaced with \"b\", \"t\" will be replaced with \"d\",",
			"and \"k\" will be replaced by \"g\". (If the first $t(charGroup)",
			"has more letters than the second, the second $t(charGroup)'s",
			"$t(run) will be repeated until it's long enough to find a",
			"match.) **NOTE:** If you have unequal numbers of $t(GR_other) in",
			"the _$t(beginning expression)_ and _$t(ending expression)_,",
			"errors may occur.",
			"",
			"The _$t(context expression)_ describes where in the word the",
			"_$t(beginning expression)_ must be before it can be changed into",
			"the _$t(ending expression)_. The _$t(exception expression)_ is",
			...
			"underscore _ represents where the _$t(ending expression)_ is",
			"being matched. You **must** include an underscore. The hash",
