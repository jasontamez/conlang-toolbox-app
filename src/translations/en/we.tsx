const we = {
	...
	info: {
		...
		charGroups: [
			...
			"**$t(wgwe:Transformations)** and **$t(Sound Changes)** to",
			"fine-tune the way your language evolves.",
			"",
			"Click the (+) button to add a new $t(wgwe:charGroup). When you",
			"make a $t(wgwe:charGroup), you must give it a",
			"_$t(common:description)_ and a one-character _$t(wgwe:label)_.",
			"The $t(common:description) is for your own benefit, while the",
			"$t(wgwe:label) will be used to refer to this $t(wgwe:charGroup)",
			"in the other tabs. The $t(wgwe:label) can be any single ",
			"character except for these: **^$\\()[]{}.*+?|**. The",
			"letters/characters in your $t(wgwe:charGroup) are called a",
			"_$t(wgwe:run)_."
		],
		...
		transformations: [
			...
			"easily.",
			"",
			"When you make a new _$t(wgwe:trans)_, you provide an",
			...
			"indicate any character in $t(wgwe:charGroup) C's $t(wgwe:run), or",
			"!%G to indicate any character _not_ in that $t(wgwe:run).)",
			"",
			...
			"$t(wgwe:trans) matches a character in a $t(wgwe:charGroup), it will",
			"note what position that character is in the $t(wgwe:charGroup)'s",
			"$t(wgwe:run). It will then look at the other expression's",
			"$t(wgwe:charGroup) and pick out the character in the same",
			"position.",
			"",
			...
			"$t(wgwe:charGroup) has more letters than the second, the second",
			"$t(wgwe:charGroup)'s $t(wgwe:run) will be repeated until it's",
			...
			"",
			"Click the (+) button to add a new $t(wgwe:trans). The first",
			"$t(wgwe:trans) in the list will be run first, the second",
			"$t(wgwe:trans) second, and so on down the list. This may cause",
			"unintended effects, so you can reorganize your transforms by",
		...
		soundChanges: [
			...
			"$t(wgwe:charGroup) C's $t(wgwe:run), or !%G to indicate any",
			"character that is _not_ in that $t(wgwe:run).)",
			"",
			...
			"matches a character in a $t(wgwe:charGroup), it will note what",
			"position that character is in the $t(wgwe:charGroup)'s",
			"$t(wgwe:run). It will then look at the _replacement_",
			"$t(wgwe:charGroup) and pick out the character in the same",
			...
			"(If the first $t(wgwe:charGroup) has more letters than the",
			"second, the second $t(wgwe:charGroup)'s $t(wgwe:run) will",
		...
		overview: [
			...
			"**$t(wgwe:Character Groups)** can hold categories of sounds that",
			"will change in the same way. **$t(wgwe:Transformations)** is a",
			"place where you can define complex $t(wgwe:trans_other) that may",
			"be needed to simplify your $t(sChange_other). (For example, you",
			"may want to simplify multi-letter sounds into a single",
			"character.) The **$t(Sound Changes)** tab is where you define",
			"the various changes you want to make, and the",
